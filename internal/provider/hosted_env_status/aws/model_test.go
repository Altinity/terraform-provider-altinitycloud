...
func TestHostedAWSEnvStatusModel_toModel(t *testing.T) {
	tests := []struct {
		name     string
		input    sdk.GetHostedAWSEnvStatus_HostedAWSEnv
		expected HostedAWSEnvStatusModel
	}{
		{
			...
			input: sdk.GetHostedAWSEnvStatus_HostedAWSEnv{
				Name:         "test-env",
				SpecRevision: 1,
				Status: sdk.GetHostedAWSEnvStatus_HostedAWSEnv_Status{
					AppliedSpecRevision: 1,
					PendingDelete:       false,
					LoadBalancers: sdk.GetHostedAWSEnvStatus_HostedAWSEnv_Status_LoadBalancers{
						Internal: sdk.GetHostedAWSEnvStatus_HostedAWSEnv_Status_LoadBalancers_Internal{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var model HostedAWSEnvStatusModel
