...
func TestAzureEnvModel_toSDK_MFA(t *testing.T) {
	tests := []struct {
		name string
		mfa  types.Bool
		want *bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			create, update, diags := AzureEnvModel{MFA: tt.mfa}.toSDK(context.Background())
			if diags.HasError() {
...
func TestAzureEnvModel_toModel_MFA(t *testing.T) {
	for _, want := range []bool{true, false} {
		var model AzureEnvModel
		if diags := model.toModel(client.GetAzureEnv_AzureEnv{Spec: &client.AzureEnvSpecFragment{Mfa: want}}); diags.HasError() {
			t.Fatalf("unexpected diagnostics: %v", diags)
		}
