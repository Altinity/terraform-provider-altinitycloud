...
func TestHCloudEnvModel_toSDK_MFA(t *testing.T) {
	tests := []struct {
		name string
		mfa  types.Bool
		want *bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			create, update, diags := HCloudEnvModel{MFA: tt.mfa}.toSDK(context.Background())
			if diags.HasError() {
...
func TestHCloudEnvModel_toModel_MFA(t *testing.T) {
	for _, want := range []bool{true, false} {
		var model HCloudEnvModel
		if diags := model.toModel(client.GetHCloudEnv_HcloudEnv{Spec: &client.HCloudEnvSpecFragment{Mfa: want}}); diags.HasError() {
			t.Fatalf("unexpected diagnostics: %v", diags)
		}
