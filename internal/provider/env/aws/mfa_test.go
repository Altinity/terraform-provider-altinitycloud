...
func TestAWSEnvModel_toSDK_MFA(t *testing.T) {
	tests := []struct {
		name string
		mfa  types.Bool
		want *bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			create, update, diags := AWSEnvModel{MFA: tt.mfa}.toSDK(context.Background())
			if diags.HasError() {
...
func TestAWSEnvModel_toModel_MFA(t *testing.T) {
	for _, want := range []bool{true, false} {
		var model AWSEnvModel
		if diags := model.toModel(sdk.GetAWSEnv_AWSEnv{Spec: &sdk.AWSEnvSpecFragment{Mfa: want}}); diags.HasError() {
			t.Fatalf("unexpected diagnostics: %v", diags)
		}
