...
func TestGCPEnvModel_toSDK_MFA(t *testing.T) {
	tests := []struct {
		name string
		mfa  types.Bool
		want *bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			create, update, diags := GCPEnvModel{MFA: tt.mfa}.toSDK(context.Background())
			if diags.HasError() {
...
func TestGCPEnvModel_toModel_MFA(t *testing.T) {
	for _, want := range []bool{true, false} {
		var model GCPEnvModel
		if diags := model.toModel(sdk.GetGCPEnv_GCPEnv{Spec: &sdk.GCPEnvSpecFragment{Mfa: want}}); diags.HasError() {
			t.Fatalf("unexpected diagnostics: %v", diags)
		}
