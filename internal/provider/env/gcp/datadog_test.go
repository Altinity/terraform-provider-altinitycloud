...
func TestDatadogToModel(t *testing.T) {
	tests := []struct {
		...
		input    *sdk.GCPEnvSpecFragment_Datadog
		...
		},
		{
			...
			existing: nil,
			input:    &sdk.GCPEnvSpecFragment_Datadog{Enabled: false, Domain: "datadoghq.com"},
			expected: nil,
		},
		{
			name:     "Unconfigured but enabled out-of-band populates without api key",
			existing: nil,
			input:    &sdk.GCPEnvSpecFragment_Datadog{Enabled: true, Domain: "us3.datadoghq.com", LogsEnabled: true, MetricsEnabled: false},
			expected: &common.DatadogModel{
		...
		},
		{
			...
			input:    &sdk.GCPEnvSpecFragment_Datadog{Enabled: true, Domain: "datadoghq.com", LogsEnabled: false, MetricsEnabled: true},
			expected: &common.DatadogModel{
				Enabled:        types.BoolValue(true),
