---
page_title: "altinitycloud_hosted_env_aws_status Data Source - terraform-provider-altinitycloud"
subcategory: ""
description: |-
...
# altinitycloud_hosted_env_aws_status (Data Source)

Altinity-hosted AWS environment status data source. It will long pool the status until `matching_spec` is `true`. Use this data source to wait for the environment is fully provisioned.

## Example Usage

### Read Altinity-hosted AWS environment status:
```terraform
...
### Wait for an Altinity-hosted AWS environment to be fully provisioned:
```terraform
