...
### HCloud environment with Datadog monitoring:
```terraform
resource "altinitycloud_env_certificate" "this" {
  env_name = "acme-staging"
}
...
- `locations` (List of String) Explicit list of HCloud locations. Currently supports single location only. ⚠️ Existing locations cannot be removed or replaced once added; only new locations may be appended.
